...
public:
    ...
    JUCE_COMRESULT get_SelectionContainer (IRawElementProviderSimple** pRetVal) override
    {
        ...
        *pRetVal = nullptr;
        ...
        auto* parentComponent = accessibilityHandler.getComponent().getParentComponent();
        ...
        while (parentComponent != nullptr)
        {
            if (auto* parentHandler = parentComponent->getAccessibilityHandler())
            {
                ...
                {
                    parentHandler->getNativeImplementation()->QueryInterface (IID_PPV_ARGS (pRetVal));
                    break;
                }
            }
...
private:
    ...
    std::vector<const AccessibilityHandler*> getSelectedChildren() const noexcept
    {
        ...
        for (auto* child : accessibilityHandler.getComponent().getChildren())
